...
async function updateAsistenciasArea() {
    console.log(`>>> Iniciando actualización de área en asistencias para la ubicación: ${ID}...`);
    
    // Referencia y Query a la colección con el filtro solicitado
    const asistenciasRef = collection(db, 'ubicaciones', ID, 'asistencias');
    const q = query(asistenciasRef, where("area", "==", AREA));

    try {
        const snapshot = await getDocs(q);
        
        if (snapshot.empty) {
            console.log(`--- No se encontraron asistencias con el área "${AREA}" en esta ubicación ---`);
            return;
        }

        console.log(`>>> Encontradas ${snapshot.size} asistencias con el área "${AREA}". Procesando actualizaciones a "${NUEVA_AREA}"...`);
        ...
        console.log('---------------------------------------------------------');
        console.log(`>>> ¡ÉXITO! Se actualizaron ${totalUpdated} registros de asistencias al área "${NUEVA_AREA}".`);
        console.log('---------------------------------------------------------');
